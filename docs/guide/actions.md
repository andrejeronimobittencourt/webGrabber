...
|--------|------------|-------------|
| `setVariable` | `key` *(str)*, `value` *(str)* | Statically bind a key-value pair into the memory buffer. Can be interpolated later via `{{key}}`. |
...
| `login` | *(See note)* | High-level macro action. Scaffolds a full 2-field login routine pointing at `usernameSelector` and `passwordSelector`. |
...
| Action | Parameters | Description |
|--------|------------|-------------|
| `log` | `message` *(str)*, `color` *(str)* | Outputs an environment logger message block mapping. Safely evaluates global `{{}}` interpolation string signatures natively. |
